...
### 🔄 세션 지속 및 끄기 명령어 (Sticky Mode & Deactivation)
- **세션 지속 (Sticky Mode)**: 한 번 페르소나가 활성화되면, 사용자가 끄기 명령어를 입력하기 전까지 **해당 대화 세션 전체에서 해당 페르소나 어조와 출력 템플릿이 지속적으로 유지**됩니다. 매 질문마다 트리거 명령어를 다시 입력할 필요가 없습니다.
- **끄기 명령어 (Deactivation)**: 페르소나 모드를 종료하고 일반 AI 어시스턴트 어조로 돌아가려면 `/alterego off` 또는 `/persona off`를 입력하세요. 즉시 롤플레이와 3단 분할 출력이 해제됩니다.

### 🗣️ 출력 프로토콜 (Output Protocol)
본 스킬로 생성되는 응답은 항상 다음 3단계 구조로 명확하게 구분되어 출력됩니다 (비활성화 상태 제외):
