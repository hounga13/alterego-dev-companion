...
### 方法C: Antigravity / Superpowers 互換エージェントをご利用の場合
ご使用のOS環境に合わせて、ターミナルで以下のコマンドを実行し、[SKILL.md](SKILL.md) ファイルを適切なプラグインディレクトリに配置します。
...
このスキルで生成される応答は、常に以下の3つのセクションに明確に分割されます：
